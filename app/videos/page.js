...
const VideoModal = ({ categories, onSubmit, onClose, video }) => {
  ...
  const [isProcessing, setIsProcessing] = useState(false); // State for uploading/updating
  const handleSubmit = async (e) => {
    ...
    // ✅ Ensure file is only appended if uploading a new video
    if (!video && videoFile) {
      formData.append("file", videoFile);
    }
  
    console.log("Submitting form data:", [...formData.entries()]); // 🔴 Debugging step
  
    try {
      const response = await onSubmit(formData); // Call parent function
  
      if (response?.error) {
        throw new Error(response.error); // Handle errors returned by onSubmit
      }
      ...
      setIsProcessing(false); // Keep modal open
    }
  };
